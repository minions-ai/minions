...
/**
 * This tool generates a stylish PDF patient report based on Q&A data. The recommendation section is wrapped so it doesn't exceed the page
 * width.
 */
...
public class PatientReportGeneratorTool implements Function<PatientReportGeneratorTool.Request, PatientReportGeneratorTool.Response> {
    ...
    try {
      ...
      try (PDDocument document = new PDDocument()) {
        ...
        try (PDPageContentStream contentStream = new PDPageContentStream(document, page)) {
          ...
          int linesUsedForCurrentMed =
              (req.currentMedications() == null || req.currentMedications().isEmpty()) ? 1 : req.currentMedications().size();
          ...
          startY = wrapText(contentStream, PDType1Font.HELVETICA, 12, req.recommendation(), margin, startY,
              page.getMediaBox().getWidth() - (margin * 2), leading);
        }
  ...
  /**
   * Writes text with word-wrapping at a specified width. Returns the new Y position after writing.
   */
