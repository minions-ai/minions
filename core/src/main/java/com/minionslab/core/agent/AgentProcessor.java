...
/**
 * AgentProcessor orchestrates the execution of an agent's workflow in the MCP framework.
...
public class AgentProcessor implements Processor<AgentContext> {
    ...
    public AgentContext beforeProcess(AgentContext input) {
        ...
        log.debug("[AgentProcessor] Starting process for agent: {}", input.getAgent().getAgentId());
    ...
    public AgentContext onError(AgentContext input, Exception e) {
        ...
        log.error("[AgentProcessor] Error processing agent: {}", input.getAgent().getAgentId(), e);
    ...
    public AgentContext afterProcess(AgentContext input) {
        ...
        log.debug("[AgentProcessor] Completed process for agent: {}", input.getAgent().getAgentId());
    ...
    public AgentContext process(AgentContext input) {
        log.info("[AgentProcessor] Processing agent workflow for agent: {}", input.getAgent().getAgentId());
