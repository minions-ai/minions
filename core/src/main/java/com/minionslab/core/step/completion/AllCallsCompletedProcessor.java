...
@Component
public class AllCallsCompletedProcessor extends AbstractProcessor<StepCompletionContext, StepCompletionOutcome> {
    
    
    @Override
    protected StepCompletionOutcome doProcess(StepCompletionContext input) throws Exception {
        if (input.getStepContext().getUnfinishedModelCalls().size() == 0) {
