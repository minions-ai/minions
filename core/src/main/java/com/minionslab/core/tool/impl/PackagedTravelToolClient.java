...
public class PackagedTravelToolClient {
    ...
    public static MethodToolCallback getcallback() {
        ...
        return new MethodToolCallback(new DefaultToolDefinition(
                ...
                inputSchema
        ), new DefaultToolMetadata(false), ReflectionUtils.findMethod(PackagedTravelToolClient.class, "call", ToolCallRequest.class), dummy,
     ...
     */
    public ToolCall.ToolCallResponse call(ToolCallRequest request) {
        List<String> cities = List.of("Paris", "London", "New York", "Tokyo", "Sydney");
        String response = String.join(", ", cities);
        return new ToolCall.ToolCallResponse(response, null);
    }
} 