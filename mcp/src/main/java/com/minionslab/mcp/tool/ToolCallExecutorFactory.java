package com.minionslab.mcp.tool;

import com.minionslab.mcp.context.MCPContext;
import com.minionslab.mcp.tool.springai.SpringAIToolCallExecutor;
import org.springframework.stereotype.Component;

@Component
public class ToolCallExecutorFactory {
    private final ToolRegistry toolRegistry;
    
    public ToolCallExecutorFactory(ToolRegistry toolRegistry) {
        this.toolRegistry = toolRegistry;
    }
    
    public ToolCallExecutor forProvider(String provider, MCPToolCall call, MCPContext context) {
        if (provider == null || provider.equalsIgnoreCase("spring")) {
            return new SpringAIToolCallExecutor(call, context,toolRegistry);
        }
        throw new IllegalArgumentException("Unknown provider: " + provider);
    }
} 